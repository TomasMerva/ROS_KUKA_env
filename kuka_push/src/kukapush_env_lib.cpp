...
std::mt19937 mt(rd());
std::uniform_int_distribution<int> x_cord(600, 980);
std::uniform_int_distribution<int> y_cord(-340, 340);
...
void kukapush_env::SetObservation(kuka_push::rl_env::Response &resp)
{
  ...
  //assign object position relative to gripper
...
{
    object_pose.position = msg->pose.position; //  object_pose.position = msg->position;
